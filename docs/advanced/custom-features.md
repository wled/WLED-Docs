...
2. Add your mode number near the end of the `#define FX_MODE_` list in `FX.h`:
`#define FX_MODE_CUSTOM                   <next_available_number>`

3. Add a PROGMEM metadata string and your effect function in `FX.cpp` (see [effect metadata](/interfaces/json-api/#effect-metadata) for the string format):
```cpp
static const char _data_FX_MODE_CUSTOM[] PROGMEM = "Custom Effect@Speed,Intensity;!,!;!;01";

void mode_custom(void) {
  // use SEGMENT, SEGENV, SEGLEN, SEGCOLOR(n), etc.
}
```

4. Register your effect in `WS2812FX::setupEffectData()` in `FX.cpp`:
`addEffect(FX_MODE_CUSTOM, &mode_custom, _data_FX_MODE_CUSTOM);`

5. Compile, upload and enjoy!  Your new effect will automatically be added to the list in the web ui.
...
class BlinkUsermod : public Usermod
{
  public:
    ...
    uint16_t getId()
    {
      return USERMOD_ID_UNSPECIFIED; // define a unique ID in const.h if needed
    }
};
```
