...
4. Wrong LED type is set. The correct LED type and the correct color sequence (RGB, BGR, etc.) must be set in WLED LED preferences. Check that these settings are correct. You have to know the LED type, but you can try out the color sequence to see which is the right one. For example, the WS2814 LED strip must be set as SK6812 and not as WS281x in the LED preferences.
