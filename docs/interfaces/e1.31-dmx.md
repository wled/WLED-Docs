...
!!! info "Art-Net Support"
    **Art-Net is also supported.** All information on this page applies for Art-Net as well.
    You will need to switch to Art-Net mode in Sync settings and reboot once.  

!!! info "DDP Support"
    **DDP is also supported.** Using DDP, the Multi RGB DMX mode is always used regardless of the DMX mode setting (as it is no DMX)
    You will need to switch to DDP mode in Sync settings and reboot once.  

!!! info "DMX Effect Channel"
...
Default mode. This is the mode you want to use for xLights and LedFx.

!!! tip "Pixel Grouping"
    You can use the segment's **Grouping** setting to treat multiple consecutive physical pixels as a single DMX channel. For example, setting Grouping to `3` means every 3 pixels share the same color and together count as one LED in the DMX channel sequence. This is useful when each DMX channel should drive a cluster of LEDs rather than a single pixel.

#### Multiple RGBW

Like `Multiple RGB` + additional white channels (4 channels per group: R, G, B, W). [Pixel Grouping](#multiple-rgb) also applies.

#### Dimmer + Multi RGB

Like `Multiple RGB`, but the first channel is a brightness control.
Master Dimmer, LED 0 Red, LED 0 Green, ... [Pixel Grouping](#multiple-rgb) also applies.

#### Dimmer + Multi RGBW

Like `Multiple RGBW`, but the first channel is a brightness control.
Master Dimmer, LED 0 Red, LED 0 Green, LED 0 Blue, LED 0 White, LED 1 ... [Pixel Grouping](#multiple-rgb) also applies.
